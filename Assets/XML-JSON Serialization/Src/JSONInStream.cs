...
public class JSONInStream
{
	...
	public JSONInStream Content(string tag, out int value)
	{
		...
		#region China
		// 绕过类型转换，在某些机型上，double到int会出现-1问题
		value = 0;

		if (fieldValue == null)
			return this;

		if (!string.IsNullOrEmpty(fieldValue.rawValue))
		{
			if (!int.TryParse (fieldValue.rawValue, out value))
				value = (int) fieldValue.value;
		}
		else if (!int.TryParse (fieldValue.Serialize(), out value))
		{
			value = (int) fieldValue.value;
		}
		#endregion
		#if DEBUG_ANDROID
		if (tag.Equals ("wId"))
			Debug.LogError ("[JSONInStream] => Content(wId)" + fieldValue.value + "|" + value);
		if (tag.Equals ("monsterId"))
			Debug.LogError ("[JSONInStream] => Content(monsterId)" + fieldValue.value + "|" + value);
		#endif
		return this;
	}
	...
	public JSONInStream Content(int idx, out int value)
	{
		...
		//	value = (int)fieldValue.value;
		#region China
		// 绕过类型转换，在某些机型上，double到int会出现-1(-0.00001)问题
		value = 0;
	...
	public JSONInStream Content(string tag, out XorInt value)
	{
		...
		if (!string.IsNullOrEmpty(fieldValue.rawValue))
		{
			if (int.TryParse (fieldValue.rawValue, out intValue))
				value = (XorInt) intValue;
		}
		else if (int.TryParse (fieldValue.Serialize(), out intValue))
		{
			value = (XorInt) intValue;
		}
		else {
  ...
  //---------------------------------------------------------------------------------
	...
	public JSONInStream Start(string tag)
	{
		try{
			...
			Debug.LogError ("[JSONInStream] => Start(" + tag + ")");
			Debug.LogError(e);
			Debug.LogError(tag);
		}
		return this;
