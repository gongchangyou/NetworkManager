...
#if DEBUG_ANDROID
        if (info.Name.Equals ("wId"))
          Debug.LogError ("[JSONSerializer] => DeserializeObject[wId]:" + resI);
		if (info.Name.Equals ("monsterId"))
			Debug.LogError ("[JSONSerializer] => DeserializeObject[monsterId]:" + resI);
#endif
        }
        break;
      case "System.Boolean":
      ...
      case "XorInt":
        ...
        if(stream.Has(info.Name))
        {
          ...
          info.SetValue(data, resXorInt);
			...
			#if DEBUG_ANDROID
			if (info.Name.Equals ("monsterId"))
				Debug.LogError ("[JSONSerializer] => DeserializeObject[monsterId]:" + resXorInt);
			#endif
        }
        break;
      default:
